...
function getTerrainGroups(terrainKey, ruleset) {
  ...
  if (!terrainGroupCache[terrainKey]) {
    ...
    terrainGroupCache[terrainKey] = terrainObj.mapBlocks.reduce(
      (acc, block, idx) => {
...
function handleBattlescapeTerrain(battlescapeTerrainData) {
  return battlescapeTerrainData?.mapBlocks?.reduce(
    (acc, block) => {
...
export function mapItemSources(backLinkSet, ruleset, key) {
  ...
  const [ufoItems, ufoRandomItems] =
    handleBattlescapeTerrain(ufos.battlescapeTerrainData) || [];
  ...
  //Items from Crafts
  const [craftItems, craftRandomItems] =
    handleBattlescapeTerrain(crafts.battlescapeTerrainData) || [];
  ...
  const armorItems = new Set([...armorSpecials, ...builtIns]);
  backLinkSet(
    ruleset.entries,
    key,
    armorItems.size > 0 && [...armorItems],
    "items",
    "$foundFrom",
  );

  //Items from deployment terrains and subterrains
  const [deploymentItems, deploymentRandomItems, customCrafts, itemSetItems] =
    getDeploymentItems(alienDeployments, ruleset);
  ...
  backLinkSet(
    ruleset.entries,
    key,
    manufactureItems && [...manufactureItems],
    "items",
    "$foundFrom",
  );
  backLinkSet(
  ...
  backLinkSet(
    ruleset.entries,
    key,
    deploymentRandomItems && [...deploymentRandomItems],
    "items",
    "$foundFrom",
  );
  backLinkSet(
...
function getManufactureItems(manufacture, ruleset) {
  const items = new Set();
  if (!manufacture.producedItems) {
    // no producedItem specified, OXC-style self-named manufacture
...
function handleCommand(
  ...
  if (command.blocks !== undefined) {
    ...
    // concat handles both array and non-arrays
    blockObjs = []
      .concat(command.blocks)
      .map((blockId) => terrainObj.mapBlocks[blockId]);
  } else {
    // groups
    const groupCache = getTerrainGroups(terrainKey, ruleset);
    // concat handles both array and non-arrays
    blockObjs = []
      .concat(command.groups || 0)
      .map((groupId) => groupCache[groupId])
      .filter((x) => x)
      .flat()
      .map((blockId) => terrainObj.mapBlocks[blockId]);
  }
  ...
  if (badBlocks.length > 0) {
    console.error(
      `Map Script ${scriptKey} referenced invalid ${terrainKey} blocks`,
    );
  }
  blockObjs
    .filter((x) => x && (x.items || x.randomizedItems))
    .forEach((block) => {
      getMapBlockItems(block, items, randomItems);
    });
}

const isRelevantCommand = (x) =>
  x.type === "addBlock" ||
...
function getDeploymentItems(alienDeployments, ruleset) {
  ...
  alienDeployments.terrains?.forEach((terrainKey) => {
    ...
    if (!baseTerrain) {
      console.error(
        `Unable to find baseTerrain "${terrainKey}" for deployment "${alienDeployments.type}"`,
      );
      return;
    }
    const scriptKey =
      alienDeployments.script ?? baseTerrain.script ?? "DEFAULT";
    const script = ruleset.lookups.mapScripts[scriptKey];

    script.commands.filter(isRelevantCommand).forEach((command) => {
      //for each command
      const commandTerrains = new Set(
        command.randomTerrain || [command.terrain || terrainKey],
      );
      ...
      commandTerrains.forEach((commandTerrainKey) => {
        //for each possible terrain for this command
        if (command.verticalLevels !== undefined) {
          //towers and shit
          command.verticalLevels.forEach((levelCommand) => {
            //for each level
            const levelTerrains = new Set(
              levelCommand.randomTerrain || [
                levelCommand.terrain || commandTerrainKey,
              ],
            );
            levelTerrains.forEach((levelTerrainKey) => {
              // for each possible terrain in the level
              handleCommand(
                scriptKey,
                levelCommand,
                levelTerrainKey,
