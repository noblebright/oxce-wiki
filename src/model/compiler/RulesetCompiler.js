...
const supportedSections = [
  // skip unrecoverable items, corpses, and unrecoverable corpses.  liveAlien seems to override recover = false.
  {
    ...
    key: "type",
    filter: (x, rs, key) =>
      (x.recover !== false || x.liveAlien === true) &&
      (x.battleType !== 11 || x.recoverCorpse !== false),
  },
  ...
  { section: "alienDeployments", key: "type" }, // deployments must be before ufos, because ufo are filtered based on deployment
  {
    section: "ufos",
    key: "type",
    filter: (x, rs, key) => rs[key].alienDeployments,
  }, // hide all the ufos that are used in site-based missions
  ...
  { section: "armors", key: "type" },
  {
    section: "alienRaces",
    key: "id",
    filter: (x, rs, key) => Object.keys(rs[key]).length > 1,
  }, //filter is run post-add, so there will always be at least one section.
...
function generateSection(ruleset, rules, metadata) {
  ...
  sectionData.forEach((entry) => {
    ...
    if (entry.delete) {
      ...
      if (
        ruleset[entry.delete] && // if there are no sections left for this key, delete the key
        !Object.keys(ruleset[entry.delete]).filter((x) => x !== "hide").length
      ) {
...
function generateAssets(ruleset, assets) {
  ...
  assets.forEach((asset) => {
    ...
    if (asset.delete && ruleset[asset.delete]) {
      //process delete
...
export default function compile(rulesList, supportedLanguages) {
  ...
  const allSoldiers = Object.keys(ruleset.entries).filter(
    (k) => ruleset.entries[k].soldiers,
  );
  ...
  for (let key in ruleset.entries) {
    ...
    backLink(
      ruleset.entries,
      key,
      ruleset.lookups.startingConditions[alienDeployments.startingCondition]
        ?.allowedCraft,
      "crafts",
      "$specialMissionAllowed",
    );
    backLink(
      ruleset.entries,
      key,
      getRandomBonusResearch(research),
      "research",
      "$randomBonusSources",
    );
    backLink(
    ...
    backLink(
      ruleset.entries,
      key,
      manufacture.producedItems && Object.keys(manufacture.producedItems),
      "items",
      "manufacture",
    );
    backLink(
      ruleset.entries,
      key,
      manufacture.requiredItems && Object.keys(manufacture.requiredItems),
      "items",
      "componentOf",
    );
    backLink(
      ...
      facilities.buildCostItems &&
        Object.keys(facilities.buildCostItems).filter(
          (k) => facilities.buildCostItems[k].build,
        ),
      "items",
      "$facilityComponent",
    );
    ...
    backLink(
      ruleset.entries,
      key,
      facilities.buildOverFacilities,
      "facilities",
      "upgradesTo",
    );
    backLink(
    ...
    backLink(
      ruleset.entries,
      key,
      armors.units || (entry.armors?.storeItem && allSoldiers),
      "soldiers",
      "usableArmors",
    );
    backLink(
    ...
    backLink(
      ruleset.entries,
      key,
      soldierTransformation.requires,
      "research",
      "$allowsTransform",
    );
    backLink(
    ...
    backLink(
      ruleset.entries,
      key,
      getKillCriteriaItems(ruleset.entries, commendations.killCriteria),
      "items",
      "$givesCommendation",
    );
    backLink(
      ruleset.entries,
      key,
      [alienDeployments.unlockedResearch],
      "research",
      "$fromMission",
    );
    backLinkSet(
    ...
    backLinkSet(
      ruleset.entries,
      key,
      soldierTransformation.requiredItems &&
        Object.keys(soldierTransformation.requiredItems),
      "items",
      "$transformComponent",
    );
    ...
    if (entry.items) {
      ...
      if (compatibleAmmo) {
        ...
        compatibleAmmo.forEach((ammo) => {
          ...
          const ammoDamageTypes = getSelfDamageTypes(
            ruleset.entries[ammo].items,
          );
    ...
    // Add category for crafts
    if (entry.crafts) {
      addToCategory(ruleset, "STR_XCOM_CRAFT_ARMAMENT", key);
    }

    backLink(
      ruleset.entries,
      key,
      entry.items?.$allCompatibleAmmo,
      "items",
      "ammoFor",
    );
    ...
    if (
      entry.alienDeployments &&
      !getPossibleRaces(key, ruleset).size &&
      entry.hide !== false
    ) {
      entry.hide = true;
    }
