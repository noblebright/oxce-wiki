...
const randomTypeHisto = [
  ...
  () => (power, resist, armor) => {
    ...
    for (let i = 0; i <= 100; i++) {
      //0-1
      const penetratingDamage = Math.floor(
        ((power * i) / 100) * resist - armor,
      );
...
class DamageSimulator {
  ...
  #getDamageHistogram(randomType, effectivePower, resist, effectiveArmor) {
    const range =
      typeof randomTypeHisto[randomType] === "function"
        ? randomTypeHisto[randomType](this.ruleset)
        : randomTypeHisto[randomType];
    ...
    for (let i = 0; i < histoLength; i++) {
      const penetratingDamage = Math.floor(
        ((effectivePower * (i + range[0])) / 100) * resist - effectiveArmor,
      );
  ...
  #getAverageDamage(powerModifier = 0) {
    ...
    const { statType, soldier, armor, target, direction, customStats } =
      this.state;
    ...
    const ignorePainImmunity = this.#getDamageAlter(
      "IgnorePainImmunity",
      false,
    );
    const randomHealthFactor = this.#getDamageAlter("RandomHealth", false)
      ? 0.5
      : 1; // RandomX is uniform 1-N distribution
    const randomStunFactor = this.#getDamageAlter("RandomStun", true) ? 0.5 : 1; // Average distribution is n / 2

    // IgnoreDirection defaults to front armor
    const { armorRating, painImmune, resist } = getTargetStats(
      entries,
      damageType,
      targetEntry,
      ignoreDirection ? "front" : direction,
      isExplosive,
    );
    const penetratingDamageMultiplier =
      this.#getDamageAlter("ToHealth", 1) * randomHealthFactor +
      this.#getDamageAlter("ToStun", 0.25) *
        (ignorePainImmunity || !painImmune ? 1 : 0) *
        randomStunFactor;
    ...
    const targetHealth =
      targetEntry.stats.health + (armorEntry.stats?.health ?? 0);
    const range =
      typeof randomTypeHisto[randomType] === "function"
        ? randomTypeHisto[randomType](this.ruleset)
        : randomTypeHisto[randomType];
    ...
    const randomBounds =
      typeof range === "function" ? [2, 1, 100] : [1, ...range];
    this.TTKParams = [
      targetHealth,
      effectiveArmor,
      ...randomBounds,
      penetratingDamageMultiplier,
    ];
    this.TTKPower = effectivePower * resist;

    return (
      this.#getDamageHistogram(
        randomType,
        effectivePower,
        resist,
        effectiveArmor,
  ...
  getRangeModifiedDamage(distance, powerModifier = 0) {
    ...
    const distanceModifier =
      -modifiedDistance * this.powerRangeObj.powerRangeReduction;
  ...
  getTTKPower(distance) {
    ...
    if (this.powerRangeObj) {
      const modifiedDistance =
        distance - this.powerRangeObj.powerRangeThreshold;
