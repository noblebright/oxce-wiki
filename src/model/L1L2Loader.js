...
async function loadRuleset({ repo, branch, path }, db, callback) {
  ...
  const [languageFiles, ruleFiles, foldCase] = await db.getFileList(
    repo,
    sha,
    path
  );
  ...
  const promises = cacheFetch.map(async (record, i) => {
    ...
    const result = YamlService.parse(text, url);
    if (!result?.[0]) {
...
export async function load(version, compiler, callback) {
  ...
  let rulesList;
  try {
    rulesList = await Promise.all(
      modules.map((m) => loadRuleset(m, db, callback))
    );
  } catch (e) {
