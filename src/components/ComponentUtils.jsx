...
import { unitStats, statNames } from "../model/Constants.js";

export const SectionHeader = ({ label }) =>
  label ? (
    <thead>
      <tr className="table-dark">
        <th className="SectionHeader" colSpan="2">
          {label}
        </th>
      </tr>
    </thead>
...
export const ContainerValue = ({ children }) => (
  <tr>
    <td colSpan="2">{children}</td>
  </tr>
);
export const SimpleValue = ({ label, value, children, showZero }) =>
  (showZero ? value || value === 0 : value) ? (
    <tr>
      <td>{label}</td>
      <td>{children ? children(value) : value}</td>
    </tr>
  ) : null;
export const BooleanValue = ({ label, value }) =>
  value !== undefined && value !== null ? (
    <SimpleValue label={label} value={`${value}`.toUpperCase()} />
  ) : null;
      ...
      <tbody>
        <tr>
          <td colSpan="2" className="ListValues">
            {values.map((val, idx) => (
              <div className="ListValue" key={idx}>
                {children ? children(val, idx) : val}
              </div>
            ))}
          </td>
        </tr>
...
function statValue({ min, max, stats }, bonusStats, field) {
  const bonusString = bonusStats[field]
    ? `[${bonusStats[field] > 0 ? "+" : ""}${bonusStats[field]}]`
    : "";
  if (stats?.[field])
    return bonusString
      ? `${stats[field]} ${bonusString} (${stats[field] + bonusStats[field]})`
      : stats[field];
  return min?.[field] && max?.[field]
    ? `${min[field]} - ${max[field]} ${bonusString}`
    : null;
}
...
export function StatRecovery({ recovery, bonusFn, lc }) {
  ...
  return (
    <React.Fragment>
      ...
      <tbody>
        {Object.keys(recovery).map((key, idx) => (
          <SimpleValue
            key={idx}
            label={lc(recoveryStrings[key])}
            value={recovery[key]}
          >
            {bonusFn}
          </SimpleValue>
...
export const ActionHeader = ({ label }) => (
  <thead>
    {label && (
      <tr className="table-secondary">
        <th className="ListHeader">{label}</th>
        <th className="ListHeader">Accuracy</th>
        <th className="ListHeader">Cost</th>
      </tr>
    )}
  </thead>
);
...
function hasMultiplier(items, suffix) {
  ...
  return (
    keys.length !== 1 ||
    keys[0] !== getDefaultAccuracy(suffix) ||
    multiplier[keys[0]] !== 1
  );
}
...
export function Accuracy({ items, suffix, bonusFn, defaultAcc }) {
  ...
  return (
    <div>
      {items[`accuracy${suffix}`] || defaultAcc}
      {hasMultiplier(items, suffix) ? ` * (${bonusFn(multiplier)})` : null}%
    </div>
  );
}
