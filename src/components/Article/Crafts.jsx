...
import useSprite from "../../hooks/useSprite.jsx";
import {
  BooleanValue,
  Hours,
  ListValue,
...
function WeaponSlots({ crafts, lc }) {
  ...
  const weaponTypes = (crafts.weaponTypes || [0, 0, 0, 0]).map((x) =>
    [].concat(x)
  );
  const weaponStrings = crafts.weaponStrings || [
    "STR_WEAPON_ONE",
    "STR_WEAPON_TWO",
  ];

  const definedWeapons = Math.min(
    crafts.weapons,
    weaponTypes.length,
    weaponStrings.length
  );
  ...
  if (
    ...
    console.warn(
      `${crafts.type} Definition mismatch for weapons, weaponTypes, and weaponStrings`
    );
  }
  for (let i = 0; i < definedWeapons; i++) {
    weaponEntry.push([
      `Weapon Slot ${i + 1}`,
      `${lc(weaponStrings[i])} (Type: ${weaponTypes[i].join(", ")})`,
    ]);
  }
  return (
    <React.Fragment>
      {weaponEntry.map(([label, value], idx) => (
        <SimpleValue key={idx} label={label} value={value} />
      ))}
    </React.Fragment>
  );
}
...
// from src/fmath.h
function getXcomDistance(nautical) {
  //go back to nm from radians
...
export default function Crafts({ ruleset, lang, id, version }) {
  ...
  const range = getXcomDistance(
    (crafts.fuelMax / 2.0 / getFuelConsumption(crafts)) *
      getRadianSpeed(crafts.speedMax) *
      120
  );
  ...
  return (
    <Table bordered striped size="sm" className="auto-width">
      ...
      <tbody>
        ...
        <SimpleValue label="Purchase Cost" value={crafts.costBuy}>
          {Money}
        </SimpleValue>
        <SimpleValue label="Monthly Buy Limit" value={crafts.monthlyBuyLimit}>
          {Money}
        </SimpleValue>
        <SimpleValue label="Rental Cost" value={crafts.costRent}>
          {Money}
        </SimpleValue>
        <SimpleValue label="Sale Price" value={crafts.costSell}>
          {Money}
        </SimpleValue>
        <BooleanValue
          label="Show in Monthly Costs"
          value={crafts.forceShowInMonthlyCosts}
        />
        <SimpleValue label="Refuel Item" value={crafts.refuelItem}>
          {linkFn}
        </SimpleValue>
        <SimpleValue
          label="Repair Rate (per 30 min)"
          value={crafts.repairRate}
        />
        <SimpleValue label="Transfer Time" value={crafts.transferTime}>
          {Hours}
        </SimpleValue>
        <SimpleValue label="Score Loss (when destroyed)" value={crafts.score} />
        <BooleanValue
          label="Keep Craft After Failed Mission?"
          value={crafts.keepCraftAfterFailedMission}
        />
        <BooleanValue label="Space Capable?" value={crafts.spacecraft} />
        <BooleanValue
          label="Notify When Refueled?"
          value={crafts.notifyWhenRefueled}
        />
        ...
        <SimpleValue
          ...
          value={Math.floor((range * 100) / 10800)}
        >
          {Percent}
        </SimpleValue>
      ...
      <ListValue label="Required Research" values={crafts.requires}>
        {linkFn}
      </ListValue>
      <ListValue label="Required Services" values={crafts.requiresBaseBuyFunc}>
        {linkFn}
      </ListValue>
      <ListValue
        label="Required To Pilot"
        values={crafts.pilotSoldierBonusesRequired}
      >
        {linkFn}
      </ListValue>
      <ListValue
        label="Special Mission Capable"
        values={crafts.$specialMissionAllowed}
      >
        {linkFn}
      </ListValue>
      <ListValue label="Available Equipment" values={[...equipment]}>
        {linkFn}
      </ListValue>
    </Table>
  );
