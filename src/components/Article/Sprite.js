import React from "react";

const MissingSprite = () => (
    <svg width="1em" height="1em" viewBox="0 0 16 16" className="bi bi-question-diamond" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        <path fillRule="evenodd" d="M6.95.435c.58-.58 1.52-.58 2.1 0l6.515 6.516c.58.58.58 1.519 0 2.098L9.05 15.565c-.58.58-1.519.58-2.098 0L.435 9.05a1.482 1.482 0 0 1 0-2.098L6.95.435zm1.4.7a.495.495 0 0 0-.7 0L1.134 7.65a.495.495 0 0 0 0 .7l6.516 6.516a.495.495 0 0 0 .7 0l6.516-6.516a.495.495 0 0 0 0-.7L8.35 1.134z"/>
        <path d="M5.25 6.033h1.32c0-.781.458-1.384 1.36-1.384.685 0 1.313.343 1.313 1.168 0 .635-.374.927-.965 1.371-.673.489-1.206 1.06-1.168 1.987l.007.463h1.307v-.355c0-.718.273-.927 1.01-1.486.609-.463 1.244-.977 1.244-2.056 0-1.511-1.276-2.241-2.673-2.241-1.326 0-2.786.647-2.754 2.533zm1.562 5.516c0 .533.425.927 1.01.927.609 0 1.028-.394 1.028-.927 0-.552-.42-.94-1.029-.94-.584 0-1.009.388-1.009.94z"/>
    </svg>
)

export default function Sprite({ ruleset, file, id, size = 1}) {
    if(size === 1) {
        if(ruleset.sprites[file]?.files[id]) {
            return <img src={ruleset.sprites[file].files[`${id}`]} className="sprite"/>;
        } else {
            return <MissingSprite/>;
        }
    } else if(size === 2) {
        console.log(id, ruleset.sprites[file].files[`${id}`]);
        return (
            <div className="sprite">
                <img src={ruleset.sprites[file].files[`${id}`]}/>
                <img src={ruleset.sprites[file].files[`${id + 1}`]} />
                <img src={ruleset.sprites[file].files[`${id + 2}`]} />
                <img src={ruleset.sprites[file].files[`${id + 3}`]} />
            </div>
        );
    }
}