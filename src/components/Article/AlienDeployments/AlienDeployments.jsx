...
import { Deployment, Reinforcements } from "./Deployment.jsx";
import EnviroEffect from "./EnviroEffect.jsx";
import StartingConditions from "./StartingConditions.jsx";
...
export default function AlienDeployments({ ruleset, lang, id, version }) {
  ...
  const enviroEffects =
    ruleset.lookups.enviroEffects[alienDeployments.enviroEffects];
    ...
    <React.Fragment>
      <Table bordered striped size="sm" className="auto-width">
        ...
        <tbody>
          <SimpleValue
            label="Map Size"
            value={`${alienDeployments.width}x${alienDeployments.length}x${alienDeployments.height}`}
          />
          <SimpleValue
            label="Civilian Count"
            value={alienDeployments.civilians}
          />
          ...
          <SimpleValue label="Next Stage" value={alienDeployments.nextStage}>
            {linkFn}
          </SimpleValue>
          <BooleanValue
            label="Final Mission"
            value={alienDeployments.finalDestination}
          />
          <SimpleValue label="Alert" value={alienDeployments.alert}>
            {lc}
          </SimpleValue>
          <SimpleValue label="Duration" value={alienDeployments.duration}>
            {(x) => `${x[0]} - ${x[1]}`}
          </SimpleValue>
          <SimpleValue label="Point Penalty" value={alienDeployments.points} />
          <SimpleValue
            label="Grants Research"
            value={alienDeployments.unlockedResearch}
          >
            {linkFn}
          </SimpleValue>
          <SimpleValue
            label="Grants Research on Failure"
            value={alienDeployments.unlockedResearchOnFailure}
          >
            {linkFn}
          </SimpleValue>
          <SimpleValue
            label="Grants Research on Despawn"
            value={alienDeployments.unlockedResearchOnDespawn}
          >
            {linkFn}
          </SimpleValue>
          <SimpleValue
            label="Grants Item"
            value={alienDeployments.missionBountyItem}
          >
            {linkFn}
          </SimpleValue>
          <SimpleValue
            label="Despawn Penalty"
            value={alienDeployments.despawnPenalty}
          />
          <SimpleValue
            label="Abort Penalty"
            value={alienDeployments.abortPenalty}
          />
          ...
          />
          <SimpleValue
            label="Mission Reward Amount"
            value={alienDeployments.missionBountyItemCount}
          />
          <SimpleValue label="Turn Limit" value={alienDeployments.turnLimit} />
          <SimpleValue label="Related Ufo" value={alienDeployments.$relatedUfo}>
            {linkFn}
          </SimpleValue>
        ...
        <ListValue label="Variant Of" values={alienDeployments.$variantOf}>
          {linkFn}
        </ListValue>
        <ListValue label="Previous Stage" values={alienDeployments.$prevStage}>
          {linkFn}
        </ListValue>
        <ListValue label="Map Items" values={mapItems}>
          {linkFn}
        </ListValue>
        <ListValue label="Map Items (Random)" values={mapRandomItems}>
          {linkFn}
        </ListValue>
        <ListValue
          label="Civilians"
          values={Object.entries(alienDeployments.civiliansByType || {})}
        >
          {inventoryFn}
        </ListValue>
        <ListValue
          label="Spawned Units"
          values={alienDeployments.$spawnedUnits}
        >
          {linkFn}
        </ListValue>
        ...
        />
        {alienDeployments.data.map((x, idx) => (
          <Deployment
            key={idx}
            ruleset={ruleset}
            linkFn={linkFn}
      ...
      <Reinforcements
        ...
        linkFn={linkFn}
        race={race}
        reinforcements={alienDeployments.reinforcements}
      />
      <Triggers
        ruleset={ruleset}
        linkFn={linkFn}
